# -*- coding: utf-8 -*-
"""
Created on Tue Jan 28 16:39:21 2025

@author: yue0004
"""

# This is the Python script to plot the spatial map of feature importance of Precipitation in a compact layout
...
#%% Plot the spatial distribution for GBR-SWE
# import FI results for GBR-SWE
# FIwith SWE
...
FI_df_withswe = FI_df_withswe_all[(FI_df_withswe_all['quantile']=='50%') & (FI_df_withswe_all['feature']=='PPT')]
...
for issue_date in forecast_date:
    ...
    plt.savefig(f'E:/USBR_Snow_Forecast/Figure/Spatial/FI/FI_map_P_withSWE_allcategories_{issue_date}.png', dpi=600, bbox_inches='tight')
    plt.savefig(f'E:/USBR_Snow_Forecast/Figure/Spatial/FI/FI_map_P_withSWE_allcategories_{issue_date}.eps', format='eps', dpi=600, bbox_inches='tight')
    plt.close()
    
    
#%% Plot the results for GBR-NoSWE
# import FI results for GBR-SWE
# FIwith SWE
FI_df_woswe_all = pd.read_csv('E:/USBR_Snow_Forecast/Results/FeatureImportance_AllData_withoutSWE.csv')
# extract results for 50% quantile
FI_df_woswe = FI_df_woswe_all[(FI_df_woswe_all['quantile']=='50%') & (FI_df_woswe_all['feature']=='PPT')]
# Define marker styles for each category
marker_styles = {
    'low': 'o',  # Circle for low flow
    'mid': '^',  # Triangle for mid flow
...
for issue_date in forecast_date:
    ...
    for category, sites in flow_categories.items():
        # Prepare data for this category
        FI_df_woswe_sid = pd.DataFrame(columns=['site_id', 'latitude', 'longitude', 'importance'])
        FI_df_woswe_sid['site_id'] = site_id_short_in_monthlyNSF

        for site_id in site_id_short_in_monthlyNSF:
            # Extract latitude and longitude from metadata
            FI_df_woswe_sid.loc[FI_df_woswe_sid['site_id'] == site_id, 'latitude'] = \
                metadata.loc[metadata['site_id_short'] == site_id, 'latitude'].values[0]
            FI_df_woswe_sid.loc[FI_df_woswe_sid['site_id'] == site_id, 'longitude'] = \
                metadata.loc[metadata['site_id_short'] == site_id, 'longitude'].values[0]

            # Extract FI importance for the site
            FI_df_woswe_sid.loc[FI_df_woswe_sid['site_id'] == site_id, 'importance'] = \
                FI_df_woswe.loc[
                    (FI_df_woswe['issue_date'] == issue_date) &
                    (FI_df_woswe['site_id'] == site_id), 'importance'
                ].values[0]

        # Filter data for current flow category
        category_data = FI_df_woswe_sid[FI_df_woswe_sid['site_id'].isin(sites)].reset_index(drop=True)
    ...
    # Set title and save the plot
    plt.title(f'NSE Feature Importance - Issue Date {issue_date}')
    plt.savefig(f'E:/USBR_Snow_Forecast/Figure/Spatial/FI/FI_map_P_withoutSWE_allcategories_{issue_date}.png', dpi=600, bbox_inches='tight')
    plt.savefig(f'E:/USBR_Snow_Forecast/Figure/Spatial/FI/FI_map_P_withoutSWE_allcategories_{issue_date}.eps', format='eps', dpi=600, bbox_inches='tight')
    plt.close()
