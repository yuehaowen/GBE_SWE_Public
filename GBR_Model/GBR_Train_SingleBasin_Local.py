# -*- coding: utf-8 -*-
"""
Created on Tue Jan 14 14:23:50 2025

@author: yue0004
"""

# This is the Python script to reproduce the single basin training of USBR forecast done by Yihan Wang
...
for date in forecast_date:
    ...
    for site in site_id_short_in_monthlyNSF:  # TODO 19 SITES FOR MONTHLY NSF site_id_short_in_monthlyNSF site_id
        ...
        #     X_train_SWE = train_df.drop(columns=['site_id', 'WY', 'volume', 'site_id_short'])
        #     X_test_SWE = test_df.drop(columns=['site_id', 'WY', 'volume', 'site_id_short'])
        #     X_train_noSWE = train_df[['PPT_acc',
        #                               'Tmax_past3',
        #                               'Tmin_past3',
        #                               'Tmax_var',
        #                               'Tmin_var',
        ...
        #         if save_model:
        #             model_filename_SWE = 'trained_models_single_basin_1982_2021/with_SWE/%s_%s_%s_%s_model.dat' % (
        #                 site, date, str(quantile), fold_name)
        #             joblib.dump(GB_regressor_SWE, model_filename_SWE)
        #             model_filename_noSWE = 'trained_models_single_basin_1982_2021/without_SWE/%s_%s_%s_%s_model.dat' % (
        #                 site, date, str(quantile), fold_name)
    ...
    for site in site_id_short_in_monthlyNSF:  # TODO 19 SITES FOR MONTHLY NSF site_id_short_in_monthlyNSF site_id
        ...
        for fold_name, fold in folds:
            ...
            for quantile in quantiles:
                ...
                GB_regressor_noSWE.fit(X_train_noSWE, y_train)
                if save_model:
                    model_filename_SWE = 'E:/USBR_Snow_Forecast/Fcst_Model/Results_Temp/single_basin/with_SWE/%s_%s_%s_%s_model.dat' % (
                        site, date, str(quantile), fold_name)
                    joblib.dump(GB_regressor_SWE, model_filename_SWE)
                    model_filename_noSWE = 'E:/USBR_Snow_Forecast/Fcst_Model/Results_Temp/single_basin/without_SWE/%s_%s_%s_%s_model.dat' % (
                        site, date, str(quantile), fold_name)
                    joblib.dump(GB_regressor_noSWE, model_filename_noSWE)
